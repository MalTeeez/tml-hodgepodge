...
//
// The name lookup alone walks the whole BlueMoon assembly. On the profiled server the two together
// were 0.329 ms of a 9.02 ms tick, and on the client 0.240 ms/frame, most of it inside
// Assembly.GetType either way. The server mod carries the same patch, but Patch there applies it
// only on a dedicated server, so on a client the cost stood unclaimed until this copy.
//
// The buffs are real gameplay -- life regen and damage -- so this patch changes how the values are
// read rather than whether they are read. Every handle is resolved once at load and the effects are
// applied as the mod applies them.
//
...
public class BlueMoonBuffLookup : Patch
{
    ...
    protected override bool Enabled => ModContent.GetInstance<ClientConfig>().BlueMoonBuffLookup;
